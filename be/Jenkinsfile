 pipeline {
    ...
    stages {
        ...
        stage('Run Test Container') {
            steps {
                script {
                    sh "docker run -d --name run-test -p 8000:8080 ${IMAGE_NAME}:v0.0.${env.BUILD_NUMBER}"
                }
            }
        }
        ...
        stage('Trigger CD Job') {
            steps {
                ...
                build job: 'paperple-be-cd', parameters: [string(name: 'IMAGE_TAG', value: "v0.0.${env.BUILD_NUMBER}")], 
                    wait: false
            }
        }
    }
